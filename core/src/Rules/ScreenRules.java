...
/**
 * Cette classe a comme objectif l'affichage de la page 1 des regles.
 */
public class ScreenRules implements Screen {
	...
	public ScreenRules(MainGame game) {
	    this.game = game;
	    ...
	    bg = new BackGroundActor("page1.jpg");
        stage.addActor(bg);
	...
	/**
	 * Cette methode permet d'afficher la premiere page des regles ainsi que les boutons en les rendant utilisables
	 * @param delta
	...
	public void render(float delta) {
		...
		if((((Gdx.input.isTouched())&&(flecheRight.isClicked(x,y))) ||(Gdx.input.isKeyPressed(Keys.RIGHT))) &&(compter<0)) {
			this.dispose();
