...
/**
 * Cette classe a comme objectif l'affichage de la page 2 des regles.
 */
public class ScreenRules2 implements Screen{
	...
	public ScreenRules2(MainGame game) {
	    ...
	    bg = new BackGroundActor("page2.jpg");
	    stage.addActor(bg);
	...
	/**
	 * Cette methode permet d'afficher la deuxieme page des regles ainsi que les boutons en les rendant utilisables
	 * @param delta
	...
	public void render(float delta) {
		...
		if((((Gdx.input.isTouched())&&(flecheLeft.isClicked(x,y)))
				...
				&&(compter<0)) {
	     	this.dispose();
	     	game.setScreen(new ScreenRules(game));
	     	volume = MainGame.vol.vols[1];
	     	SONtransition.play(volume);
		 }

		if((((Gdx.input.isTouched())&&(flecheRight.isClicked(x,y)))
				||(Gdx.input.isKeyPressed(Keys.RIGHT)))
				&&(compter<0)) {
	     	this.dispose();
	     	game.setScreen(new ScreenRules3(game));
	     	volume = MainGame.vol.vols[1];
	     	SONtransition.play(volume);
		}
